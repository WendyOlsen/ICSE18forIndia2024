/*------------------------------------------------------
File: 01-extraction-plfs2022-23.do
Purpose: To Extract PLFS 2022-23
...
// census adjusted weight 

destring state_ut sector, replace
gen inflator = .
replace inflator =1.14624123581376 if state_ut==1 & sector==1
replace inflator =1.20641280529967 if state_ut==2 & sector==1
replace inflator =1.16279129463477 if state_ut==3 & sector==1
replace inflator =0.187129192990085 if state_ut==4 & sector==1
replace inflator =1.19687270002471 if state_ut==5 & sector==1
replace inflator =1.11656267176179 if state_ut==6 & sector==1
replace inflator =0.571984268449818 if state_ut==7 & sector==1
replace inflator =1.26003120472297 if state_ut==8 & sector==1
replace inflator =1.25099102684757 if state_ut==9 & sector==1
replace inflator =1.20772422935461 if state_ut==10 & sector==1
replace inflator =0.959787028563639 if state_ut==11 & sector==1
replace inflator =1.22932072207552 if state_ut==12 & sector==1
replace inflator =0.827100112089502 if state_ut==13 & sector==1
replace inflator =1.29131169660537 if state_ut==14 & sector==1
replace inflator =1.19067904141969 if state_ut==15 & sector==1
replace inflator =1.07945005921035 if state_ut==16 & sector==1
replace inflator =1.15653054819047 if state_ut==17 & sector==1
replace inflator =0.349750917975163 if state_ut==18 & sector==1
replace inflator =1.04176553232352 if state_ut==19 & sector==1
replace inflator =1.19983214717487 if state_ut==20 & sector==1
replace inflator =1.23162729988907 if state_ut==21 & sector==1
replace inflator =1.15173767872566 if state_ut==22 & sector==1
replace inflator =1.21889110938525 if state_ut==23 & sector==1
replace inflator =1.09775306077015 if state_ut==24 & sector==1
replace inflator =0.804016920051081 if state_ut==25 & sector==1
replace inflator =1.13304127841224 if state_ut==27 & sector==1
replace inflator =1.07494471306999 if state_ut==28 & sector==1
replace inflator =1.08266866971278 if state_ut==29 & sector==1
replace inflator =0.77158586494642 if state_ut==30 & sector==1
replace inflator =0.371573961018807 if state_ut==31 & sector==1
replace inflator =0.703082809431256 if state_ut==32 & sector==1
replace inflator =1.0396229880159 if state_ut==33 & sector==1
replace inflator =1.14683391274321 if state_ut==34 & sector==1
replace inflator =1.09347203663973 if state_ut==35 & sector==1
replace inflator =2.48477463796003 if state_ut==1 & sector==2
replace inflator =1.24782329946819 if state_ut==2 & sector==2
replace inflator =1.51572704275303 if state_ut==3 & sector==2
replace inflator =1.84823976518148 if state_ut==4 & sector==2
replace inflator =1.93078840302633 if state_ut==5 & sector==2
replace inflator =1.61795514885623 if state_ut==6 & sector==2
replace inflator =2.12865252874527 if state_ut==7 & sector==2
replace inflator =1.46085083421061 if state_ut==8 & sector==2
replace inflator =1.61473408323008 if state_ut==9 & sector==2
replace inflator =2.00384686682363 if state_ut==10 & sector==2
replace inflator =3.94599945109191 if state_ut==11 & sector==2
replace inflator =2.35560289045765 if state_ut==12 & sector==2
replace inflator =2.13985523536052 if state_ut==13 & sector==2
replace inflator =1.63365561729534 if state_ut==14 & sector==2
replace inflator =1.68922505006793 if state_ut==15 & sector==2
replace inflator =3.48966046341054 if state_ut==16 & sector==2
replace inflator =1.68812318189593 if state_ut==17 & sector==2
replace inflator =1.89380388164933 if state_ut==18 & sector==2
replace inflator =1.52229136451608 if state_ut==19 & sector==2
replace inflator =2.04633897543874 if state_ut==20 & sector==2
replace inflator =1.7291844975311 if state_ut==21 & sector==2
replace inflator =1.88587943941606 if state_ut==22 & sector==2
replace inflator =1.49723555187441 if state_ut==23 & sector==2
replace inflator =1.38308570489094 if state_ut==24 & sector==2
replace inflator =2.82754727677138 if state_ut==25 & sector==2
replace inflator =1.48935420116238 if state_ut==27 & sector==2
replace inflator =1.54226366728556 if state_ut==28 & sector==2
replace inflator =1.53185441396654 if state_ut==29 & sector==2
replace inflator =1.6156811414246 if state_ut==30 & sector==2
replace inflator =2.43847197184643 if state_ut==31 & sector==2
replace inflator =2.27569194052266 if state_ut==32 & sector==2
replace inflator =1.60712094178654 if state_ut==33 & sector==2
replace inflator =1.54348965093962 if state_ut==34 & sector==2
replace inflator =1.130465239283 if state_ut==35 & sector==2

gen census_adj_weight = a_weight * inflator
gen i_census_adj_weight = round( census_adj_weight,1)

tab sector [iwe = i_census_adj_weight]

// To get census weights deflator for rural and urban areas based on overall population. Numerator is UN projection and denominator is the estimates from rounded census weights. We follow UN's projection: https://www.macrotrends.net/countries/IND/india/population-growth-rate

gen census_deflator=.
replace census_deflator = 908804812/943356740 if sector==1  
replace census_deflator = 508368361/510110964 if sector==2

//These are the corrected census weights after deflating using rural and urban deflators, respectively.
gen census_corrected_weight = census_adj_weight* census_deflator 

//These are the final weights used in the study.
gen i_census_corrected_weight = round(census_corrected_weight,1)

tab sector 
tab sector [iwe = i_weight]
tab sector [iwe = i_census_corrected_weight]
